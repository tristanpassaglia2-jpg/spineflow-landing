(() => {
  ...
  function renderProgram(region, path, program) {
    ...
    return `<button class="btn btn-dark back-button" data-back-path>← Volver a patologías</button>
      ...
      <div class="card" style="margin-bottom:20px;background:linear-gradient(135deg,#0f8b84,#0d7772);color:#fff;padding:20px 24px;box-sizing:border-box"><div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:16px"><div style="min-width:0;flex:1"><p style="opacity:.85;font-size:.85rem;margin:0;line-height:1.2">Tu progreso</p><div style="margin:6px 0 0;color:#fff;font-size:1.4rem;line-height:1.2;font-family:var(--serif);font-weight:560">${doneCount} de ${totalSessions} sesiones</div></div><div style="min-width:140px;flex-shrink:0"><div style="height:8px;background:rgba(255,255,255,.25);border-radius:10px;overflow:hidden"><div style="height:100%;width:${pct}%;background:#fff;transition:width .3s"></div></div><p style="margin:6px 0 0;font-size:.8rem;opacity:.9;text-align:right">${pct}%</p></div></div></div>
      ${evaChart}${weeksHtml}`;
  }
  ...
  function exercise() {
    ...
    const seqNode = state.sequences[state.currentExercise];
    const isPendingImage = seqNode && seqNode.status === 'pending_image';
    const firstImg = imgs ? imgs[0] : `media/exercises/${state.currentExercise}.webp`;
    const loopStageHTML = isPendingImage
      ? `<div class="loop-stage" id="loopStage" style="background:linear-gradient(135deg,#e8f0ea,#d4e4d8);color:#1a3a2a;display:flex;align-items:center;justify-content:center;padding:32px;text-align:center;min-height:320px"><div style="max-width:400px"><div style="font-size:56px;margin-bottom:16px">📋</div><h3 style="margin:0 0 12px 0;font-size:1.1rem">Este ejercicio se guía con voz y pasos</h3><p style="opacity:.75;font-size:.9rem;margin:0">Escuchá la guía y seguí los pasos numerados que están a la derecha</p></div></div>`
