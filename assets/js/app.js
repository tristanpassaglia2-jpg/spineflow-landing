(() => {
  ...
  function renderProgram(region, path, program) {
    ...
    return `<button class="btn btn-dark back-button" data-back-path>← Volver a patologías</button>
      ...
      <div class="card" style="margin-bottom:20px;background:linear-gradient(135deg,#0f8b84,#0d7772);color:#fff;padding:20px 24px;box-sizing:border-box"><div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:16px"><div style="min-width:0;flex:1"><p style="opacity:.85;font-size:.85rem;margin:0;line-height:1.2">Tu progreso</p><h2 style="margin:6px 0 0;color:#fff;font-size:1.4rem;line-height:1.2">${doneCount} de ${totalSessions} sesiones</h2></div><div style="min-width:140px;flex-shrink:0"><div style="height:8px;background:rgba(255,255,255,.25);border-radius:10px;overflow:hidden"><div style="height:100%;width:${pct}%;background:#fff;transition:width .3s"></div></div><p style="margin:6px 0 0;font-size:.8rem;opacity:.9;text-align:right">${pct}%</p></div></div></div>
      ${evaChart}${weeksHtml}`;
  }
