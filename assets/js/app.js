(() => {
  ...
  function exercise() {
    ...
    return `<button class="btn btn-dark back-button" data-exercise-back>← Volver al programa</button><div class="exercise-layout"><section class="card exercise-main"><div class="exercise-title-row"><div><p class="eyebrow">Ejercicio guiado por Mi Profe</p><h1>${ex.name}</h1></div><button class="btn btn-light" data-favorite>${fav?'♥ Guardado':'♡ Favorito'}</button></div><div class="exercise-tags"><span class="pill">${ex.level}</span><span class="pill">${ex.series}</span><span class="pill">${ex.reps}</span></div>
      <div class="player"><div class="player-head"><div class="coach-mini"><img src="media/coach/mi-profe.webp" alt="Mi Profe"><div><strong>Mi Profe</strong><span>Guía de movimiento</span></div></div><div class="timer" id="timer">00:${String(state.seconds).padStart(2,'0')}</div></div><div class="phase-frame"><img id="phaseStrip" class="phase-strip" src="media/exercises/${state.currentExercise}.webp" alt="Secuencia fotográfica de ${ex.name}" style="transform:translateX(-${state.phase*20}%)" onload="this.parentElement.style.aspectRatio=(this.naturalWidth/5)+' / '+this.naturalHeight"><span class="phase-caption" id="phaseCaption">${phases[state.phase]}</span></div><div class="phase-dots">${phases.map((p,i)=>`<button class="phase-dot ${i===state.phase?'active':''}" data-phase="${i}">${p}</button>`).join('')}</div><div class="player-controls"><button class="btn btn-light" data-play>▶ Iniciar</button><button class="btn btn-light" data-voice>◉ Voz guiada</button><button class="btn btn-primary" data-complete>✓ Completar</button></div></div><div class="instruction"><small id="stepLabel">Indicación · fase ${state.phase+1}</small><p id="stepText">${step}</p></div></section>
